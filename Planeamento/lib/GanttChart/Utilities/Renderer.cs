...
namespace GanttChart
{
    public class Renderer
    {
        ...
        private void InitDefaultValues()
        {
            ...
            TopHeaderHeight = 40;
            DefaultTimeLabelFormat = "HH";
            DefaultDayLabelFormat = "dd/MM/yyyy";
            StartHourInDay = 0;
            EndHourInDay = 24;
        ...
        private void PopulateDateTimeXLocs(int startX, int endX)
        {
            ...
            while (curDay < EndDate)
            {
                ...
                //int MinDivision = (int)Math.Round((double)(timeDivision / 60));
                int xLoc = dayStart;
                ...
                while (curHour <= EndHourInDay)
                {
                    ...
                    else
                    {
                        ...
                        //{
                        //    xMin += MinDivision;
                        //    TimeXLocations.Add(new Tuple<DateTime, int, StringFormat>(dateWithHour.AddMinutes(min), xMin, otherFormat));
                        //}
        ...
        private void DrawChart()
        {
            ...
            if (AreHoursValid() && AreDatesValid())
            {
                ...
                Rectangle leftRect = DrawLeftHeaders(0, TopHeaderHeight + topPading);

                PopulateDateTimeXLocs(leftRect.Right, autoScrollMinSize.Width);

                Rectangle topRect = DrawTopHeaders(leftRect.Right, topPading, autoScrollMinSize.Width, TopHeaderHeight + 1 + topPading);
        ...
        private Rectangle DrawTopHeaders(int startX, int startY, int endX, int endY)
        {
            ...
            {
                if (timeLabelInfo.Item1.Hour % 2 == 0)
                {
        ...
        {
            float xMin = (float)(TimeXLocations[1].Item2 - TimeXLocations[0].Item2)/60;
            foreach (Row row in Rows.Where(p => p.IsVisible))
            {
                foreach (TimeBlock timeBlock in row.TimeBlocks.Where(p => p.IsVisible))
                {
                    ...
                    float timeBlockStartX = GetXLocationForTime(timeBlock.StartTime) + 1;
                    timeBlockStartX += timeBlock.StartTime.Minute * xMin;
                    float timeBlockEndX = GetXLocationForTime(timeBlock.EndTime) - 1;
                    timeBlockEndX += timeBlock.EndTime.Minute * xMin;

                    int timeBlockY = row.Rect.Y + 5;
                    RectangleF rect = new RectangleF(timeBlockStartX, timeBlockY, timeBlockEndX - timeBlockStartX, 20);
        ...
        {
            DateTime truncatedDateTime = new DateTime(dateAndTime.Year, dateAndTime.Month, dateAndTime.Day, dateAndTime.Hour, 0, 0);
            Tuple<DateTime, int, StringFormat> foundGroup = TimeXLocations.Find(p => p.Item1 == truncatedDateTime);
            if (foundGroup != null)
                return foundGroup.Item2;
            else
                return -1;
        ...
        private void DrawRect(RectangleF rect, Color insideColor, Color borderColor, bool offsetForPen = false, bool hatch = false)
        {
        ...
        private void DrawTextLeft(RectangleF rect, string text, Color? textColor = null)
        {
            int stringLengthPixels = (int)Math.Round(graphics.MeasureString(text, this.font).Width);
            if (stringLengthPixels > rect.Width - 3) //"3" leaves room for margin on both sides
            {
            ...
            Point center = new Point((int)rect.X + 3, (int)rect.Y + (int)rect.Height / 2);
        ...
        public List<Tuple<DateTime, int, StringFormat>> TimeXLocations { get; set; } //Format is "Date (with hour), xLocation, alignment"l
