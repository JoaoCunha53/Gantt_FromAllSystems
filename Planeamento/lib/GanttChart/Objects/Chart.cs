...
namespace GanttChart
{
    public class Chart : UserControl
    {
        ...
        private void doubleClickTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (milliseconds >= SystemInformation.DoubleClickTime)
            {
                ...
                foreach (Row row in Rows.Where(p => p.IsVisible))
                {
                    ...
                    {
                        clickedRow = row;
                        FormMaquinaEdit formMaquina = new FormMaquinaEdit(row);
                    ...
                    else if (row.TimeBlocks.Find(p => p.Rect.Contains(doubleClickLocation)) != null)
                    {
                        ...
                        FormExistTimeBlock formExistTimeBlock = new FormExistTimeBlock(Rows, clickedTimeBlock,row.Text,Rows.IndexOf(row));
                        formExistTimeBlock.ShowDialog();
                        break;
                    }
                }
        ...
        public bool AddRow(Chart chart, int rowId, string rowName,int idTimeBlock, string TimeBlockText,string description, DateTime startTime, DateTime endTime, int r,int g,int b)
        {

            if (chart.Rows.Exists(r => r.id.Equals(rowId)) == false)
            {
                Row newRow = new Row(rowId, rowName);
                newRow.idChargesBook = chart.Id;
                newRow.TimeBlocks.Add(new TimeBlock(idTimeBlock, TimeBlockText, description, startTime, endTime) { Color = Color.FromArgb(r, g, b) });
                chart.Rows.Add(newRow);
                return true;
            }
            else
            {
                int index = chart.Rows.FindIndex(r => r.id.Equals(rowId));
                chart.Rows[index].TimeBlocks.Add(new TimeBlock(idTimeBlock, TimeBlockText, description, startTime, endTime) { Color = Color.FromArgb(r, g, b) });
                return false;
            }
        }

        public bool AddRow(Chart chart, int rowId, string rowName)
        {
            if (chart.Rows.Exists(r => r.id.Equals(rowId)) == false)
            {
                Row newRow = new Row(rowId, rowName);
                newRow.TimeBlocks = new List<TimeBlock>();
                chart.Rows.Add(newRow);
                return true;
            }
            return true;
        ...
        public bool VerifyClickOnTimeBlock()
        {
            foreach(Row row in Rows)
            {
                foreach(TimeBlock timeblock in row.TimeBlocks)
                {
                    if (doubleClickLocation.X >= timeblock.Rect.X 
                        && doubleClickLocation.X <= timeblock.Rect.X + timeblock.Rect.Width
                        && doubleClickLocation.Y >= timeblock.Rect.Y
                        && doubleClickLocation.Y >= timeblock.Rect.Y + timeblock.Rect.Height)
                    {
                        return true;
                    }
                }
