...
namespace Calendar
{
    public class CalendarChart : UserControl
    {
            ...
            if (milliseconds >= SystemInformation.DoubleClickTime)
            {
                ...
                foreach (Row row in Rows.Where(p => p.IsVisible))
                {
                    ...
                    else if (row.TimeBlocks.Find(p => p.Rect.Contains(doubleClickLocation)) != null)
                    {
                        ...
                        FormEditHolidays fromEditHolidays = new FormEditHolidays(row, clickedTimeBlock);
                        fromEditHolidays.ShowDialog();
        ...
        public bool AddRow(CalendarChart calendarChart, int month, string rowName, int idTimeBlock, string TimeBlockText, string description, int year,int day, int r, int g, int b,bool hatch)
        {

            if (calendarChart.Rows.Exists(r => r.Month.Equals(month)) == false)
            {
                Row newRow = new Row(month, rowName);
                newRow.TimeBlocks.Add(new TimeBlock(idTimeBlock, TimeBlockText, new DateTime(year,month,day), description, hatch) { Color = Color.FromArgb(r, g, b) });
                calendarChart.Rows.Add(newRow);
                return true;
            }
            else
            {
                int index = calendarChart.Rows.FindIndex(r => r.Month.Equals(month));
                calendarChart.Rows[index].TimeBlocks.Add(new TimeBlock(idTimeBlock, TimeBlockText, new DateTime(year, month, day), description, hatch) { Color = Color.FromArgb(r, g, b) });
                return false;
            }
        }

        public bool AddRow(CalendarChart calendarChart, int month, string rowName)
        {
            if (calendarChart.Rows.Exists(r => r.Month.Equals(month)) == false)
            {
                Row newRow = new Row(month, rowName);
